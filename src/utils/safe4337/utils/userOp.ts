...
export const calculateSafeOperationHash = (
  erc4337ModuleAddress: string,
  safeOp: SafeUserOperation,
  chainId: BigNumberish,
): string => {
  return ethers.TypedDataEncoder.hash(
    { chainId, verifyingContract: erc4337ModuleAddress },
    EIP712_SAFE_OPERATION_TYPE,
    safeOp,
  );
};

export const calculateSafeOperationData = (
  erc4337ModuleAddress: string,
  safeOp: SafeUserOperation,
  chainId: BigNumberish,
): string => {
...
export const buildSafeUserOp = (
  template: OptionalExceptFor<
    SafeUserOperation,
    'safe' | 'nonce' | 'entryPoint'
  >,
): SafeUserOperation => {
...
export const buildSafeUserOpTransaction = (
  ...
  const method = bubbleUpRevertReason
    ? 'executeUserOpWithErrorString'
    : 'executeUserOp';
  const callData = new ethers.Interface(abi).encodeFunctionData(method, [
    to,
    value,
    data,
    delegateCall ? 1 : 0,
...
export const getRequiredGas = (userOp: UserOperation): string => {
  ...
  return (
    ...
    BigInt(userOp.verificationGasLimit) * multiplier +
...
export const getSupportedEntryPoints = async (
  ...
  const supportedEntryPoints = await provider.send(
 ...
 */
export const packValidationData = (
  ...
  const result =
    addrBigInt |
    (validUntilBigInt << BigInt(160)) |
    (validAfterBigInt << BigInt(160 + 48));
